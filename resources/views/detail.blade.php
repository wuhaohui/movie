...
@section('main')
    <div id="main">
        ...
        <div class="view">
            ...
            <div class="info"><h1>{{ $movie->name }}</h1>
                <ul>
                    <li><span>状态：
                            @if($movie->episodes)
                                 </span><font color="#009999">更新至{{ $movie->now_episodes }}
                            集</font>/共{{ $movie->episodes }}集
                    </li>
                    @else
                    ...
                    <li><span>类型：</span><a
                                href="/list/16.html">{{ $movie->type }}</a>　　<span>语种：</span>{{ $movie->language }}</li>
                    <li><span>地区：</span><a
                                href="/dq/2.html">{{ $movie->language }}</a>　　<span>上映时间：</span>{{ $movie->show_time }}
                    </li>
                    <li>
                        <div class="fl"><span>更新日期：</span>{{ $movie->updated_at }}</div>
                        <a class="fr" href="#pl" style="text-align:right;"><img src="images/pl.gif" alt="查看评论"
                                                                                width="90"
                                                                                height="22"/></a></li>
                </ul>
                ...
                <div class="pfen"><p>影片评分：</p>
                    <div class="starscore"><input type="hidden" id="MARK_B1" name="MARK_B1" value="0"/><input
                                type="hidden"
                                id="MARK_B2"
                                name="MARK_B2"
                                value="0"/><input
        ...
        <div class="endpage clearfix"><a id="kan"></a>
            ...
            <div class="comment">
                <div class="ds-thread" data-thread-key="pv_395"
                     data-title="《{{ $movie->name }}》{{ $movie->type }}在线观看,{{ $movie->name }}网盘迅雷下载 - 飘V网"
